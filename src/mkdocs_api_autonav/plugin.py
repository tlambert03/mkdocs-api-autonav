    ...
    def on_nav(
        ...
        if not config.nav:
            ...
            for item in nav.items:
                ...
                # if we get here, there was no config.nav declared *and* our
                # autgenerated files above were not found in the autogenerated nav.
                # this may mean that the user has additional navigation plugins, like
                # mkdocs awesome nav.  We need to add our section to the end of the nav.
                root = self.nav.as_mkdocs_item(config=config)
                root.title = self.config.nav_section_title
                nav.items.append(root)
        return nav
...
class _NavNode:
    ...
    def as_mkdocs_item(self, config: MkDocsConfig) -> StructureItem:
        """Convert the tree to a mkdocs nav object."""
        if not self.children:
            # No children, return a page
            if self.file is None:  # pragma: no cover
                raise RuntimeError(
                    "File is None, but no children exist. This should not happen."
                )
            return Page(title=self.full_title, file=self.file, config=config)

        return Section(
            title=self.full_title,
            children=[
                child_node.as_mkdocs_item(config=config)
