...
@pytest.mark.skipif(
    not Path(C_PROJECT).exists(),
    reason="C project fixture not found"
)
def test_build_c_project_integration(c_project: Path):
    ...
    try:
        ...
        assert "api-autonav" in plugin_names or any("api-autonav" in str(p) for p in cfg.plugins)
...
def test_backwards_compatibility_python():
    ...
    cfg = {
        ...
        "plugins": [
            ...
            {
                "api-autonav": {
                    "modules": ["src/my_library"],  # Legacy format
